...
from config import (
    TTS_VOICE_A, TTS_VOICE_B, TTS_RATE_A, TTS_RATE_B,
    TTS_PITCH_A, TTS_PITCH_B, AUDIO_DIR,
...
VOICE_MAP = {
    MC_A: (TTS_VOICE_A, TTS_RATE_A, TTS_PITCH_A),
    MC_B: (TTS_VOICE_B, TTS_RATE_B, TTS_PITCH_B),
}
...
async def synthesize_line(text: str, voice: str, rate: str, pitch: str, output_path: str):
    """1行のセリフを音声化"""
...
async def generate_all_lines(script: list[tuple[str, str]], temp_dir: str) -> list[str]:
    ...
    for i, (speaker, text) in enumerate(script):
        voice, rate, pitch = VOICE_MAP.get(speaker, (TTS_VOICE_A, TTS_RATE_A, TTS_PITCH_A))
        path = f"{temp_dir}/line_{i:04d}.mp3"
        try:
            await synthesize_line(text, voice, rate, pitch, path)
            paths.append(path)
