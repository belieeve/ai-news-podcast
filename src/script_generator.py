...
- 明日からできる具体的な行動（「今すぐ〜に登録しましょう」「明日から〜を試しましょう」など）は提案しないこと。
- ニュースレター（メルマガ）への案内や誘導は一切行わないこと。
...
=== SUMMARY ===
（ポッドキャスト概要欄文。今週のひと言まとめ、採用したニュース一覧、各ニュースを採用した理由、ニュース/ツールリンクプレースホルダ、Spotifyフォローのお願い、最後の締めを含む。※ニュースレターの案内や出典元の記述は入れないこと）

=== SCRIPT ===
（掛け合い台本。全行は必ず「{mc_a}:」または「{mc_b}:」で始める。演出指示・注釈・空行は入れない。質問文は必ず「か」で終える。構成通りに作成し、最終行に <<END>> とだけ書く）

=== SNS_DRAFT ===
【ニュースレター導入文】（今週のAIニュース全体の導入文。AI副業ログや今週の学びのまとめ、無料版要約・活用メモ形式。※案内ではなく、今週のまとめ記事的なテキスト）
...
- 構成ルール（厳守）:
  ...
  7. クロージング: エンディング挨拶。ニュースレター（メルマガ）への案内や誘導、ニュースの出典元の紹介は一切含めず、シンプルな挨拶だけで終えてください。
- エンディング例：
