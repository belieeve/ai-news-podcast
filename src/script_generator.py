...
【番組コンセプト】
- 番組名: {show_name}
- サブタイトル: 「AIを軸に、ビジネス・エンタメ・SNSのバズまで、毎朝10分の情報収集」
...
- 番組の目的は、今日話題になっていることを10分で把握できる状態にすることです。行動の提案（ToDo）は必須ではありません。
- 最後（クロージング）は短く、以下の流れで締めてください。
  1. 今日いちばん覚えておきたい話題を一言でまとめる。
  2. 番組フォローへの自然な案内を入れる。
  3. 「また明日」の挨拶で短く終わる。
  - 例：
    「今日をひとことでまとめると、◯◯が大きく動いた一日でした。」
    「この番組では、AIからエンタメまで、毎朝の気になる話題を10分でお届けしています。よければ番組のフォローもお願いします。」
    「それでは、また明日の朝お会いしましょう。」
...
- MC: {mc_a}
  - 役割：その日の話題を整理し、「なぜ話題なのか」「知っておくと何の役に立つのか」を判断する。知的好奇心を刺激しながら、テンポよく番組を進める。
- MC: {mc_b}
  - 役割：もう一人のMCとして、日常や視聴者目線で会話を広げる。驚き役だけにせず、「それはなぜ今バズっているんですか」「普段の生活や仕事に関係ありますか」「知らない人に一言で説明するとどうなりますか」「これは追いかけたほうがいい話題ですか」のような質問で、話題をリスナーの理解に近づける。
...
- 完成音声が8分〜12分に収まる長さにする。
- 台本全体は日本語で必ず2,600〜3,600文字にしてください（2,400文字未満は不可）。テンポよく聴きやすい文章にしてください。
- 構成ルール（厳守）:
...
- エンディング例：
    {MC_A}: 今日をひとことでまとめると、AIとエンタメの両方で大きな動きがあった一日でした。
    {MC_B}: この番組では、AIからエンタメまで、毎朝の気になる話題を10分でお届けしています。よければ番組のフォローもお願いします。
    {MC_A}: それでは、また明日の朝お会いしましょう。
"""
