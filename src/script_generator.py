...
- サブMC（リスナー代表・質問役）: {mc_b}
  - 役割：会社員や個人クリエイターの視点で質問する普通の会社員代表。専門用語が出たときに噛み砕く質問をしたり、リスナーの疑問（例：「普通の会社員にも関係ある？」「副業をしている人は何から試せばいい？」「今すぐ使うべき？それとも様子見？」「難しそうだけど、簡単に言うとどういうこと？」）を代弁する相棒。

【会話と掛け合いのルール】
- {mc_a}（解説役）が解説し、{mc_b}（質問役）が噛み砕いて質問する構造を徹底してください。リスナーは{mc_b}に自分を重ねて聴きます。
- キャラクターの掛け合いを無駄に増やしすぎず、リスナーが内容を理解しやすくなることを最優先としてください。雑談ではなく、理解を深めるために会話を使ってください。
- 冒頭の名乗り（リニューアル直後の設定）として、お互いの肩書きが伝わる以下の挨拶を取り入れても良いです（通常時はシンプルに「{mc_a}です」「{mc_b}です」とします）。
  - 「AIニュースを仕事目線で読み解く{mc_a}です」
  - 「リスナー代表として、普通の会社員目線で質問していく{mc_b}です」
