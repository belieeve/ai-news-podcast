...
【この番組の文体について（重要）】
この番組はギャルMC2人（{mc_a}・{mc_b}）の掛け合いで、口調がギャル語であることは番組の仕様です。
- 「やば」「えぐい」「まじで」「それな」「神」「優勝」「レベチ」「アゲ」「沼る」「推せる」「じわる」「とりま」「〜じゃん」「〜なんよ」などのギャル語は、話し手自身の感想やリアクションである限り、煽り・誇張とみなさず、絶対に標準語や堅い表現に直さないでください。ギャル口調をおとなしくする方向の修正は行わないでください。
- 修正するのは、あくまで「事実の誤り」「報道ベースを公式発表と言い切っている」「効果や安全性の断定」「読者を煽って行動させる表現」だけです。
- ただし次の場合はギャル語であっても修正対象です：(1)企業・製品・サービス・人物そのものへの断定的な評価になっている場合（例「この会社もう終了」「これ使わない人は詰んだ」）、(2)実在の人物の容姿や恋愛に関する表現、(3)死を連想させる表現（無事死亡・キュン死・メンタル死ぬ・心臓止まる・HPゼロなど）、(4)音声で意味が通らない文字専用の表現（草・www・クソワロタ・オワタ・むりぽ・り・りょ）。これらは意味を保ったまま、別のギャル語のリアクションに置き換えてください（標準語に戻すのではなく、ギャル口調のまま直す）。
...
3. 誇張・煽り・断定の排除：「全面的に安全」「完全に解決」「終焉」「禁止」「確定」「仕事がなくなる」「必ず」「絶対に」「AI三国志」「完全に安全」「ChatGPT一強の終焉」「時代遅れ」などの表現がないか。ただし、ニュースの事実ではなく話し手の気持ちを表すギャル語（「沼確定」「勝ち確」「◯◯しか勝たん」など）はこれに当たらないので修正しないこと。
